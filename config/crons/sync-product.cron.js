...
module.exports.start = async function (data={}) {
  ...
  let cron_name = is_sync_all ? 'CRON_SYNC_ALL_PRODUCT' : 'CRON_SYNC_PRODUCT';

  let query = is_sync_all ? {} : getSellerQueryTime({ fetchWindowMinutes, bufferMinutes });
  let isDeleteOtherProduct = is_sync_all; 
  ...
  await ShopModel.find({status: 1}).lean(true).cursor().eachAsync(async shop => {
    ...
    const [error, result] = await to(ProductService.syncSeller({ shop, query, isDeleteOtherProduct }));
