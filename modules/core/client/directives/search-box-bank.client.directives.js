...
angular.module('core').directive('searchBoxBank', function($timeout, Gateways) {

  var template="";
  template += "<div class=\"box-search-advance\">";
  template += "  <div>";
  template += "    <input class=\"form-control placeholder-black border-none-radius\" placeholder=\"{{ placeholder }}\" ng-model=\"searchText\" ng-change=\"search()\" ng-disabled=\"disabled\" ng-focus=\"suggestionBoxToggle()\" ng-click=\"suggestionBoxToggle()\"\/>";
  template += "  <\/div>";
  ...
  template += "            <label class=\"inline_block\" ng-bind=\"item[titleProp]\"><\/label>";
  template += "            <div class=\"clear\"><\/div>";
  template += "            <span ng-bind=\"item[subtitleProp]\"></span>";
  template += "            <div class=\"clear\"><\/div>";
  template += "          <\/li>";
  template += "        <\/ul>";
  template += "      <\/div>";
  ...
  return {
    ...
    link: function(scope, elem, attrs) {
      ...
      var query = function() {
        ...
        resource.queryPaymentGateway(params, function(data) {
